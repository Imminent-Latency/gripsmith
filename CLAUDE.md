...
catches it. **Stage explicitly (`git add <the row's paths>`) — never `git add -A`**: `CLAUDE.md`, `HANDOFF.md` and
`docs/` are all untracked, and only P0 commit 1 should take them. There is no `origin` remote yet; the user
adds it. End every commit message with: `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`

...
npx tsc -p tsconfig.app.json --noEmit 2>&1 | grep 'error TS' \
  | sed -E 's/\(([0-9]+),([0-9]+)\)//' | sort > /tmp/tsc-after-$PHASE.txt
...
npx tsc -p tsconfig.app.json --noEmit 2>&1 | grep 'error TS' \
  | sed -E 's/\(([0-9]+),([0-9]+)\)//' | sort > /tmp/tsc-before-$PHASE.txt
```

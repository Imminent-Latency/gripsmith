...
| Remotes | `git remote -v` | `upstream` only. **No `origin`** at baseline — added alongside P0 commit 4: `origin` → `git@github.com:Imminent-Latency/gripsmith.git` (default branch `gripsmith`; `upstream` is never reused as `origin`). `pnpm deploy:gh` (`package.json:12`) still waits on `base` (P2 commit 18). |
...
| `src/components/controls/BaseControls.tsx` | P2 (commits 19, 20, 22, 23), P7 (commit 73) | Merge in phase order. **P7 commit 73 replaces `:143-156` with `SwatchGrid`** — commit 78 (the two remaining bare label groups) does **not** touch this file — its groups are `GeometryControls.tsx:219-222` and `InlayControls.tsx:438-441`; the `BaseControls` colour-grid group comes free from `SwatchGrid` (as built, P7). |
...
| **12** | 7 | `feat(react): thread seed through the seven-hop chain` | Copies `rotationClamp` hop for hop: `ModelViewer` destructure + **JSX attribute**; `ImperativeModelProps` + body destructure; `buildJob`; **dependency array**. | `src/components/ModelViewer.tsx:48-54`, `:460-501`; `src/components/ImperativeModel.tsx:15-54`, `:60-96`, `:843-849`, `:897-903` | `GATE` — **the tsc diff is the only gate that can catch this commit's two failure modes** (and it must be the normalised diff: this commit's `ModelViewer.tsx:48-54` edit sits above the pre-existing `ModelViewer.tsx(89,10)` error); **A9** `awk '/^  \}, \[$/,/^  \]\);/' src/components/ImperativeModel.tsx \| grep -c '\bseed\b'` → **1** (the opener is anchored with `$`: unanchored, `^  \}, \[` also matches the one-line dependency arrays earlier in the file, the range opens early and counts `buildJob`'s `seed` too — 2 on a correct commit) — **symbol-anchored, because this commit's own insertions at `:15-54`, `:60-96` and `:843-849` push the dependency array below `:897`.** On the untouched tree the array is `}, [` at `:897` … `]);` at `:903`; a hardcoded `sed -n '897,904p'` can report 0 on a correct commit | green |
| **13** | 8 | `feat(inlay): seed the main-thread inlay placement` | **Read §5.1 row 17 first: the call passes only 12 arguments today.** Insert `null` for `avoidShapes` (argument 13), *then* `item.seed ?? DEFAULT_SEED` (argument 14) — copy the shape from commit 8's A13 test. Verify `InlayJob` is untouched. | `src/components/ImperativeModel.tsx:254-267` | `GATE`; **A3a** — **a phase-scope criterion evaluated *after* this commit lands, not a working-tree diff.** **As built, the phase-range `git diff -U0 <P0-tip>..HEAD … \| grep -E '^\+\+\+'` form is unsatisfiable**: by this row six files under those paths have changed (rows 6–12), so it names six, and `-U0` hides a call opener whose only change is its arguments. Verify instead: (1) `git diff --name-only HEAD~1..HEAD -- src/utils/geometry src/components` → exactly one file, `src/components/ImperativeModel.tsx` (the `patternPipeline.ts` call at `:184` was seeded in commit 11); (2) `grep -rn 'generateTilePositions(' src/utils/geometry src/components --include='*.ts' --include='*.tsx' \| grep -v '\.test\.'` → exactly two hits; (3) print each call from its opener to its **actual** closing parenthesis (the pipeline call closes inside a ternary, not with `);`) and confirm both pass `seed` as argument 14, with `null` for `avoidShapes` immediately before it here. Doc 08 `:445` states A3a the same way. **A4** `git diff src/utils/geometry/inlayPipeline.ts` is **empty** | green |
...
| Seq | Task | Commit message | What it does | Files | Verify | Green |
|---|---|---|---|---|---|---|
| **15** | 1 | `refactor(outline): move PatternPreset and PRESETS to src/constants` | Verbatim move; re-export both from `PatternLibraryModal.tsx`; add `'src/constants/**'` to the coverage allowlist. **No behaviour change.** | `src/constants/presets.ts` (new), `src/components/PatternLibraryModal.tsx`, `vite.config.ts:30-35` | `GATE`; A1's regression half only (no existing test lost) — A1's five-file delta is a **phase-exit** assertion evaluated at row 24, because the five files do not exist yet at this row. **As built: an inserted commit 15a `chore(deps): add @types/node for the filesystem-based outline tests (doc 01 §A3)` lands before row 16** — doc 01's specimen test needs `fs`/`path`/`process`; the tsc baseline loses exactly the three `projectUtils.test.ts` TS2304 `global` lines and gains nothing, and the post-chore list becomes the phase baseline | green |
...
| **69** | A-4 | `feat(params): ParamField for number, select, toggle, segmented` | `src/components/params/ParamField.tsx` (new) + T2, T3. **As built: an inserted prerequisite commit `feat(ui): forward id on ToggleButton and SegmentedControl` lands before this row** — the render-function form needs `id`-accepting primitives that row 79 would otherwise deliver too late; it forwards `id` only, and row 79 keeps the convention work | `GATE`; **T3 red-then-green demonstrated**; T2's four kinds green | green |
...
| **71** | A-6 | `feat(params): read-only ParamProvider` | `src/context/ParamContext.tsx` (new — `ParamProvider` + the read hook, per doc 04 §4.2.2 `:199` and its file map `:496`) + `src/context/ParamContext.test.tsx` (new; `src/context/**` is on the coverage allowlist); `src/components/Controls.tsx:368`; `InlayControls.tsx` (shim deleted); `src/components/params/labels.test.tsx` (fixture only — deleting the shim makes `InlayControls` a context consumer, so every fixture that renders it must mount `ParamProvider`) | `GATE`; derived bounds resolve from the provider with **no** locally-constructed `ParamContextValue` and **no** new props; the A-5 shim is gone | green |
| **72** | A-7 | `feat(ui): Slider primitive` | `src/components/ui/Slider.tsx` (new) + test; `ParamField` kind `slider`; `src/components/controls/GeometryControls.tsx` (the one migrated field — the shared-file table at `:385` already assigns this file to commit 72) | `GATE`; first slider = **Rotate / `baseRotation`, min 0, max 360, step 15** (decision of record: the only Geometry field with an intrinsic range — the schema declares none, and every other candidate needs an invented maximum or carries an `Auto`/`None` sentinel a slider cannot express; human-overridable before P7 merges), shipping in the Geometry panel on the same 150 ms debounce; T2's `slider` assertion green | green |
...
| **76** | A-11 | `feat(a11y): modal dismiss and picker tiles as buttons` | `src/hooks/useModalDismiss.ts` (new); `src/components/PatternLibraryModal.tsx:160-166` + T6 | `GATE`; A4's `aria-pressed` is **prop-driven** — an optional `selectedFile?: string` on `PatternLibraryModalProps` (keyed on `preset.file`) renders the attribute only when defined and omits it otherwise; never last-clicked state, never `interactiveFile` (the 3D-preview toggle). The three callers stay unwired in this row because nothing in the app knows the current preset yet; the P7 a11y follow-up queued behind PR #6 wires `selectedFile` once P2 row 17's ids have merged. Keyboard walkthrough passes for **all three** pickers (`BaseControls.tsx:65`, `GeometryControls.tsx:177`, `InlayControls.tsx:270` — one component, three `category` values) | green |
...
| **79** | A-14 | `style(ui): one active-state and focus-ring convention` | `Button`, `ToggleButton`, `SegmentedControl`, swatches; `ui/Badge.tsx` deleted (doc 04 §4.2.5's "use it or delete it" decision) | `GATE`; the grep criterion in doc 04 §6.3. **Isolated commit** — it changes the tab bar | green |
| **80** | A-15 | `test(params): regeneration guard` | new test (`src/utils/params/regenerationGuard.test.ts`; read the two components as text via Vite `?raw` imports — `node:fs` needs `@types/node`, which only P2's chore 15a adds) | `GATE`; **T7** — green today (check (c) asserts the JSX **forward**, see doc 04 `:268`); **red** in a scratch commit that skips the dep array, **and** red in a second scratch commit that names the dep array but skips the `ModelViewer` forward (cross-cutting rule 4) | green |

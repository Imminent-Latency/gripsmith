...
- **`public/CNAME` names the upstream author's domain.** It contains `studio.grippysheet.com`. `gh-pages -d dist` copies `public/` into the published branch, so deploying as-is would attempt to claim someone else's custom domain. **Delete `public/CNAME` in M0.** *Done — P0 commit 4 (`5cd73ce`).*
- **`gh-pages` defaults to an `origin` remote, which the fork did not have.** `git remote -v` listed only `upstream`; `origin` → `git@github.com:Imminent-Latency/gripsmith.git` was added alongside P0 commit 4 (`upstream` stays and must never be reused as `origin`). `pnpm deploy:gh` (`package.json:12`) now waits only on `base` (next bullet; doc 01's `BASE_URL` task).

...
| [`02-generator-engine`](./02-generator-engine.md) | Creates `ShapeSource` and `Generator<P>` + a lazy registry that writes `THREE.Shape[]` into `patternShapes` — the first production caller of `buildJob`'s `kind:'shapes'` branch. Reuses the existing worker; adds no job kind. | Ready to implement — D1 adopted 2026-09-04 (root §5.2 rule 5 amended by P0 commit 1) | **M3** | M | M0 for the deletions · M2 for `seed` |
...
| [`05-direct-editing`](./05-direct-editing.md) | **05a:** inlay region addressing, 3D pick, durable region edits, and the three lossy paint-modal defects. **05b:** pattern node/region editing — blocked, recommend dropping. | Draft. 05a buildable · **05b deferred, not scheduled** | **M5** | 05a M · 05b XL | §4.4 (lossy-edit fixes): nothing. D3: doc 07 §4.5 adopted; §4.3 and T5–T8 struck. 05b: a polygon representation for the pattern, which does not exist. |
...
Solid arrows are hard blocks. Dashed arrows are soft ordering (root §6 phasing). Hexagons are adopted amendments and decisions (D1–D3, @liamstar, 2026-09-04).
...
graph TD
  ...
  Q1{{"adopted D1: rule-5 carve-out<br/>main-thread generation"}}
  Q2{{"adopted D2: rule-2 carve-out<br/>insetConvex"}}
  Q3{{"adopted D3: persistence<br/>doc 07 zip asset"}}
  ...
  classDef adopted fill:#245c39,stroke:#5ba878,color:#fff;
  classDef deferred fill:#3a3a3a,stroke:#777,color:#ddd,stroke-dasharray:4 3;
  class Q1,Q2,Q3 adopted;
  class D05B deferred;
```

**Reading the graph.** Four docs can start today with no predecessor: **01**, **06a**, **08**, and **04**'s half A. The rest is ordinary sequencing (`08 → 02 → 03`, `01 → 04` half B) with the three D1–D3 decisions adopted on 2026-09-04 and recorded by P0. **02 follows its code prerequisites; its root-doc blocker is resolved.** Only `05b` is blocked on capabilities that do not exist: doc 05 §4.7 enumerates five preconditions, one of which (P3, stable per-region identity through `Manifold.compose`) is owned by nobody and would need a §10 amendment of its own. It is recommended for removal from the roadmap rather than carried as a phantom XL.

---

## Decisions of record

Rows 1–3 are decisions D1–D3 adopted by @liamstar on 2026-09-04. Row 4 is the D5 editorial resolution, not a dated ruling by @liamstar. Each is recorded in the doc that needs it.

| # | Decision | Gates | Recorded in |
|---|---|---|---|
| 1 | **D1 — adopted by @liamstar, 2026-09-04:** root §5.2 rule 5 permits main-thread polygon synthesis under the stated budget and revisit trigger. | doc 02 task 1 root-doc blocker resolved | doc 02 §3 (rule-5 row), §4.5 |
| 2 | **D2 — adopted by @liamstar, 2026-09-04:** root §5.2 rule 2 permits `insetConvex` under all four convex-inset conditions. | doc 03 task 2 amendment adopted | doc 03 §8 Q6 |
| 3 | **D3 — adopted by @liamstar, 2026-09-04:** doc 07 §4.5's synthetic `.shapes.json` zip asset ships; all three doc 04 `shapes` keys and doc 05's `editedShapes` mechanism are struck. Edited geometry supersedes its source asset. | persistence mechanism resolved | doc 05 §8 question 6 · doc 04 §4.3.1 · doc 07 §4.5 |
| 4 | **Resolved editorially by D5 — root §5.1 amended by P0 commit 1:** `ShapeSource` lives in `src/utils/generators/types.ts`; a `Generator` synthesises the unit and the existing tiler places it. **Not a dated @liamstar ruling.** | wording only; no code is blocked | doc 02 §3, §4.2 |

...
| E2 | `seed = NaN`, a float, or a negative, from a hand-edited bundle | Floats and negatives are accepted by the schema (no `.int()`, §4.4) and coerced by `Math.trunc(seed) >>> 0`. **`NaN` never reaches the schema — JSON cannot carry it and `z.number()` rejects it — so the schema stays `z.number().default(1)`**; the `NaN >>> 0 === 0` coercion lives at the PRNG level, where a NaN seed degrades to stream 0, never to `NaN` coordinates. |
...
| **A3a** | Only two **production** call sites are seeded, one per commit: `git diff --name-only HEAD~1..HEAD -- src/utils/geometry src/components` at the inlay-placement commit → exactly `src/components/ImperativeModel.tsx` (the `src/utils/geometry/patternPipeline.ts` call at `:184` is seeded in the pipeline commit); `grep -rn 'generateTilePositions(' src/utils/geometry src/components --include='*.ts' --include='*.tsx' \| grep -v '\.test\.'` → exactly two hits, each printed to its real closing parenthesis and ending with `seed` as argument 14. (The phase-range `git diff -U0 … \| grep -E '^\+\+\+'` form cannot pass: six files under those paths change earlier in the phase, and `-U0` omits an unchanged call opener.) |

...
### 4.3 Persistence — `editedShapes`

> **Overlap notice — this section is one of three competing mechanisms; it is not normative until @liamstar picks one.** An earlier draft of this heading read "(normative for doc 07 as well)". It is not. The same hole — geometry with no source file must survive export → import — is scoped with a different mechanism in **doc 04 §4.3.1** (`ProjectSchemaV2.shapes.inlayShapes`, inline in `project.json`, keyed by item id) and again in **doc 07 §4.5** (`serializeShapeAsset` → a synthetic `<name>.shapes.json` zip entry). All three edit the same rehydration block at `src/components/Controls.tsx:206-231` and all three declare a **`SerializedShapeSchema` (new)** in `src/types/schemas.ts` — doc 04 by that exact name. **Only one ships.** Doc 04 §4.3.1 carries the comparison table and owns the single declaration of `SerializedShapeSchema`; root §6 assigns persistence-for-source-less-shapes to doc 07 and `ProjectSchemaV2` to doc 04. Do not build this section until the choice is made (§8 Q6). What is *not* in dispute and belongs to this doc either way: §4.4's three lossy-edit defects and the recentre compensation.
...
export const SerializedShapeSchema = z.object({          // (new — does not exist yet)
...
| `src/components/controls/InlayControls.tsx` | `:322-331` | Compensate the recentre into `item.x`/`item.y`; write `editedShapes` | The only `onSave` handler |
| `src/components/controls/InlayControls.tsx` | `:118-156` | `handleShapeUpload` must clear `editedShapes` when a **new file** replaces the layer | Loading a new asset must not resurrect stale edits |
| `src/types/schemas.ts` | `:19-46` | `SerializedShapeSchema`, `EditedInlayShapeSchema`, `editedShapes` — all **(new)** | One schema file; root §5.2 rule 6 budgets exactly this |
| `src/utils/projectUtils.ts` | `:48-50` | Strip live `shapes`, keep `editedShapes` | The only bundle writer |
...
6. **`positionPreset`.** Scale and rotate recompute `x`/`y` from `calculateInlayOffset` when the preset is not `'manual'` (`InlayInteractionHandles.tsx:218-233`, `:249-264`). An `editedShapes` write changes the shape bounds, so it changes the preset offset. Recompute or pin — do not leave it stale.
...
| `src/utils/projectUtils.test.ts` *(extend)* | An item with `editedShapes` survives `exportProjectBundle` → `importProjectBundle`; the exported `project.json` contains **no** `"curves"` key (proof the THREE blob is gone) |
| `src/utils/inlay/shapeEdit.test.ts` | `structuredClone(editedShapesArray)` succeeds — the rule-5 guard |
...
7. **Round trip, observable:** open a layer in the paint modal, drag one shape, save, export the bundle, re-import it. The dragged position is preserved. *(Today it reverts to the source file — `Controls.tsx:226`.)* Backed by a `projectUtils.test.ts` assertion on `editedShapes`.
...
14. Importing a **v1 bundle produced before this change** still loads: items rehydrate from their assets exactly as today, and `editedShapes` reads `null`.
...
| **T5** | Add `SerializedShapeSchema`, `EditedInlayShapeSchema`, `InlayItemSchema.editedShapes` **(all new)**, each with `.default()`. Extend `schemaDefaults.test.ts`. | `schemas.ts:19-46` | — |
| **T6** | Export: strip live `shapes`, keep `editedShapes`. | `projectUtils.ts:48-50` | T5 |
| **T7** | Import: apply §4.3's precedence rule; relax the missing-asset warning. | `Controls.tsx:206-231`, `:132-141` | T5, T6 |
| **T8** | Write `editedShapes` on paint-modal save; clear it when a new file replaces the layer. Round-trip test. | `InlayControls.tsx:322-331`, `:118-156` | T4, T7 |
...
| **T11** | Region actions — recolour and delete — writing through `editedShapes`, with the tile-fan-out disclosure of edge case 2. Uses `worldToItemLocal` / `pointInShapeWithHoles` for the *where in the shape* half. | `InlayRegionPicker.tsx`; `InlayControls.tsx` | T8, T9, T10 |
...
2. **Should `editedShapes` supersede the source asset, or diff against it?**
   §4.3 specifies *supersede* — simplest, and it makes the bundle self-describing. The cost: a user who edits a layer then wants the pristine file back has no route except re-uploading, since import stops consulting the asset for that item.
   *Settles it:* a decision on whether "Revert to source" is a wanted affordance. If yes, keep the asset in the bundle (it already is) and add a button that clears `editedShapes` — cheap, but it must be designed in T7, not retrofitted.
...
6. **Which of the three persistence mechanisms ships?** *(Cross-document; blocks §4.3 and doc 07 §4.5 and doc 04 §4.3.1's `shapes.inlayShapes` block.)*
   The same hole — geometry with no source file must survive export → import — is scoped three times, over the same rehydration block at `src/components/Controls.tsx:206-231`: **doc 04 §4.3.1** (`ProjectSchemaV2.shapes.inlayShapes`, inline in `project.json`), **this §4.3** (`InlayItemSchema.editedShapes`, inline on the item), **doc 07 §4.5** (a synthetic `<name>.shapes.json` zip asset). Docs 04 and 05 both declare a `SerializedShapeSchema` **(new)** in `src/types/schemas.ts` under that exact name. Root §6 names persistence-for-source-less-shapes as doc 07's row and `ProjectSchemaV2` as doc 04's; root §8 M5's *"a painted or traced inlay survives export → import"* bullet is claimed by this doc's §4.3 **and** by doc 07's header. Root does not adjudicate the mechanism. Trade-offs: the zip-asset route (07) is the only one that also covers the **pattern** lane (07 §4.5a) and reuses the existing asset transport; the inline routes (04, 05) make `project.json` self-describing and need no sniffer change, but 05's is inlay-only and duplicates 04's schema work.
   *Settles it:* @liamstar picks one, and the other two docs delete their mechanism and cite the winner. Nothing here is blocked on code — the decision is the blocker. §4.4's lossy-edit fixes are unaffected and can ship first either way.

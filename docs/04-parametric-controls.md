...
| A4 | **Picker tiles are buttons.** `<div onClick>` → `<button type="button">` with `aria-pressed` for the current selection — **prop-driven**: the modal takes an optional `selectedFile` (keyed on `preset.file`) and renders the attribute only when it is defined, omitting it otherwise; it never infers selection from the last click or from the 3D-preview toggle, because the callers load asynchronously and close the modal before the load resolves. Wiring `selectedFile` from the three callers needs a selected-preset source of truth (P2's stable ids) and is owned by the P7 a11y follow-up queued behind PR #6, after P2 merges. | `src/components/PatternLibraryModal.tsx:160-166` | This is **all three** pickers — outlines, inlays and patterns use the same modal, filtered by `category` (`src/components/controls/BaseControls.tsx:65` `category="outlines"` at `:68`; `GeometryControls.tsx:177`, which passes none and takes the `'patterns'` default at `PatternLibraryModal.tsx:75`; `InlayControls.tsx:270` `category="inlays"` at `:273`). |
...
3. the identifier is forwarded as a JSX attribute named exactly `<key>` on `<ImperativeModel>` in `src/components/ModelViewer.tsx:460-501` with a value derived from `geometrySettings` — either the identifier destructured at `:48-54` or a `geometrySettings.<key>` expression. (Two fields, `baseRotation` at `:479` and `patternMaxHeight` at `:481`, have always been forwarded directly; the forward is what matters — check 1 alone cannot see a prop `ModelViewer` never forwards — and normalising those two destructures belongs to the P7 a11y follow-up queued behind PR #6, after P1 merges, since P1 edits the same block.)
...
| T5 | `src/components/params/labels.test.tsx` | Render `BaseControls`, `GeometryControls`, `InlayControls` and assert **two** things, separately named so a task can satisfy one without claiming the other. **T5a:** every `<label>` rendered *by `ControlField`* (`src/components/ui/ControlField.tsx:27`) whose child is a native form control, a `ParamField`, or a custom component that forwards `id` has an `htmlFor` resolving to an element in the same document; the cases A1's guard cannot reach — the three `ToggleButton` sites (`BaseControls.tsx:132`, `GeometryControls.tsx:511`, `InlayControls.tsx:635`) and any wrapper-`<div>` field until it is migrated — are **enumerated as exclusions** in the test, and the count of resolved labels ≥ the count of `ControlField`s rendered minus those exclusions. (As built: `ToggleButton`/`SegmentedControl` forward `id` since P7's inserted prerequisite, but the guard is native-only and those callers pass no `id`, so their exclusions stand until the P7 a11y follow-up queued behind PR #6 widens the guard or migrates the callers.) **T5b:** every `<label>` **not** rendered by `ControlField` sits inside an element carrying `role="group"` and an accessible name — the four groups of §4.2.5 A7 (`BaseControls.tsx:144`, `GeometryControls.tsx:220`, `:550`, `InlayControls.tsx:439`). A single unconditional "every `<label>` has a resolving `htmlFor`" is **unachievable by design**: those four label a *set* of controls, and `ShapeUploader.tsx:180` already has its `htmlFor` while its `ControlField` child is a wrapper `<div>` (`:179`) that A1's injection cannot reach. |
...
| A-6 | `ParamProvider` (read-only) mounted at `Controls.tsx:368`, deriving `selectedInlayItem` per I1; **the A-5 shim in `InlayControls` is deleted** | A-5 | derived bounds resolve from the provider with no locally-constructed `ParamContextValue` and no new props; the shim is gone from `InlayControls`. Deleting it makes `InlayControls` a context consumer, so every existing fixture that renders it (T5's `labels.test.tsx`) mounts `ParamProvider` in the same commit — wrapper only, assertions unchanged |
| A-7 | `ui/Slider.tsx` + `ParamField` kind `slider` + the first migrated field in `GeometryControls.tsx` | A-4 | first slider is **Rotate / `baseRotation`, 0–360, step 15** (decision of record, §4.2.7 — human-overridable before P7 merges); commits on the same 150 ms debounce; T2's `slider` assertion green |

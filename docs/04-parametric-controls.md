...
| A-7 | `ui/Slider.tsx` + `ParamField` kind `slider` + the first migrated field in `GeometryControls.tsx` | A-4 | first slider is **Rotate / `baseRotation`, 0–360, step 15** (decision of record, §4.2.7); commits on the same 150 ms debounce; T2's `slider` assertion green |
